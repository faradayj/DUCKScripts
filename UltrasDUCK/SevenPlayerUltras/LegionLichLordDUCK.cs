...
public class LegionLichLordDUCK
{
    ...
    private const string LogPrefix = "LegionLichLordDUCK";
    private const string SyncFileName = "LegionLichLordDUCK.sync";
    private const string MapName = "frozenlair";
    private const string SafeCell = "Enter";
    private const string SafePad = "Spawn";
    private const string BossCell = "r3";
    private const string BossPad = "Left";
    private const string LegionPromotion = "Legion Promotion";
    private const string BossName = "Legion Lich Lord";
    private const int UltraQuestId = 1674;
    ...
    public string OptionsStorage = "LegionLichLordDUCK";
    public bool DontPreconfigure = true;
    public List<IOption> Options = new()
    {
        new Option<int>(
            "SapphireOrbQuantity",
            "Sapphire Orb Quantity",
            "0 = Daily Quest 1674 only (Single Kill). 1-600 = Continuous Farm until Sapphire Orbs reach this quantity.",
            0
        ),
    ...
    private static readonly DuckSlotRequirement[] LichLordSlots = new[]
    {
    ...
    private void ExecuteScript()
    {
        ...
        int targetQuantity = masterMode ? 0 : Bot.Config?.Get<int>("SapphireOrbQuantity") ?? 0;
        ...
        Duck.FileLog($"Started as {playerAlias} in room {privateRoomNumber} (Mode: {(isDailyMode ? "Daily Quest 1674" : $"Farm {targetQuantity} Sapphire Orbs")}).", LogPrefix);
        Core.Logger($"{LogPrefix} started as {playerAlias} in room {privateRoomNumber} (Mode: {(isDailyMode ? "Daily Quest 1674" : $"Farm {targetQuantity} Sapphire Orbs")}).");

        Core.Unbank(LegionPromotion);
        Bot.Drops.Add("Lineage of Devastation", "Sapphire Orb");
    ...
    private bool PrepareRighteousSeal(int fightAttempt)
    {
        ...
        while (!Bot.ShouldExit)
        {
            ...
            {
                Core.Logger($"{LogPrefix} {playerAlias} confirmed Righteous Seal on Legion Lich Lord.");
                return true;
            }
    ...
    {
        string signal = $"LICH_SEAL_READY_{fightAttempt}";
        if (!Duck.SendArmySignal(signal))
            return false;

        Core.Logger($"{LogPrefix} {playerAlias} broadcasted {signal}.");
        return true;
    }
    ...
    private bool RunContinuousDropFarm(ClassPreset preset, int targetQuantity)
    {
        ...
        string readySignal = "SAPPHIRE_ORB_GOAL_REACHED";
        ...
        while (!Bot.ShouldExit)
        {
            if (!Bot.Player.Alive)
            {
                if (!deathLogged)
                {
                    Core.Logger($"{LogPrefix} {playerAlias} died during Sapphire Orb farm.");
                    deathLogged = true;
                }
            ...
            if (!readySignalSent && Duck.CheckItemQuantity("Sapphire Orb", targetQuantity))
            {
                ...
                Core.Logger(
                    $"{LogPrefix} {playerAlias} reached {targetQuantity} Sapphire Orbs and continues fighting to assist teammates."
                );
            }

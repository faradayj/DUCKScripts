/*
name: UltraWardenv4
description: Ultra Warden v4 — timer-based 2-taunter rotation with army synchronization.
tags: null
*/
...
public class UltraWardenv4
{
        ...
        private void EquipPresetClasses()
    {
        ...
        SlotRequirement[] slotRequirements = UltraEnhancementsv4.GetSlotRequirements("warden", UltraClassesByRole);

        UltraCustomClassSyncv4.CustomClassSync(Ultra, Bot, slotRequirements, armySize, "ultra_warden_class-v4.sync", allowDuplicates);
    }

    private void Prep()
    {
        // Clear stale class registrations from previous runs/other accounts
        Ultra.ClearSyncFile(Ultra.ResolveSyncPath("ultra_warden_class-v4.sync"));
        ...
        Enh.ApplyContext("warden");

        C.Logger($"[UltraWarden-v4] Role: {_role} ({className})");
    }
    ...
    private void Fight()
    {
        ...
        if (skipThird)
        {
            C.Logger("[UltraWarden-v4] Taunter detected, equipping Scroll of Enrage.");
            Engine.EquipEnrage();
        }
        ...
        if (_role == "Taunter1")
        {
            C.Logger("[UltraWarden-v4] Taunter1 (Primary) — setting fight start time.");
        ...
        {
            C.Logger("[UltraWarden-v4] Taunter2 (Secondary) — reading fight start time.");

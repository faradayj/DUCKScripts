/*
name: UltraGramielv4
description: Ultra Gramiel v4 — two-phase fight (crystals then Gramiel) with chat-driven crystal taunt warnings and pulse-driven taunt rotation.
tags: null
*/
...
public class UltraGramielv4
{
        ...
        try
        {
            while (_deathRetries.Value < MaxDeathRetries)
            {
                ...
                UltraDeathv4.StartWipeMonitor(
                    ...
                    () => UltraDeathv4.PerformRetreat(C, 4, MaxDeathRetries, _deathRetries, "UltraGramielRetreat.sync")
                );
    ...
    private void ApplyTaunterClasses()
    {
        ...
        if (className.Equals(LeftCrystalT1, StringComparison.OrdinalIgnoreCase))
        {
            ...
            C.Logger($"[UltraGramiel-v4] Assigned to taunt LEFT crystal (mapId=2) - T1 (Gramiel taunter index 0)");
        }
        else if (className.Equals(LeftCrystalT2, StringComparison.OrdinalIgnoreCase))
        {
            ...
            C.Logger($"[UltraGramiel-v4] Assigned to taunt LEFT crystal (mapId=2) - T2 (Gramiel taunter index 1)");
        }
        else if (className.Equals(RightCrystalT1, StringComparison.OrdinalIgnoreCase))
        {
            ...
            C.Logger($"[UltraGramiel-v4] Assigned to taunt RIGHT crystal (mapId=3) - T1 (Gramiel taunter index 2)");
        }
        else if (className.Equals(RightCrystalT2, StringComparison.OrdinalIgnoreCase))
        {
            ...
            C.Logger($"[UltraGramiel-v4] Assigned to taunt RIGHT crystal (mapId=3) - T2 (Gramiel taunter index 3)");
        }
        else
        {
            crystalMapId = 2;
            ...
            isT1Taunter = true;
            _gramielTaunterIndex = 0;
            C.Logger($"[UltraGramiel-v4] Class '{className}' not a taunter, defaulting to LEFT crystal DPS.");
        }
    }
        ...
        private void EquipPresetClasses()
    {
        ...
        SlotRequirement[] slotRequirements = UltraEnhancementsv4.GetSlotRequirements("gramiel", UltraClassesByRole);

        UltraCustomClassSyncv4.CustomClassSync(Ultra, Bot, slotRequirements, armySize, "ultra_gramiel_class-v4.sync", allowDuplicates);
    }

    private void Prep()
    {
        // Clear stale class registrations from previous runs/other accounts
        Ultra.ClearSyncFile(Ultra.ResolveSyncPath("ultra_gramiel_class-v4.sync"));
        ...
        Enh.ApplyContext("gramiel");

        C.Logger($"[UltraGramiel-v4] Class: {Bot.Player.CurrentClass?.Name ?? "None"} | IsTaunter: {IsTaunter()}");
    }
    ...
    private void Fight()
    {
        ...
        if (skipThird)
        {
            C.Logger("[UltraGramiel-v4] Taunter detected, equipping Scroll of Enrage.");
            Engine.EquipEnrage();
        }
        ...
        while (!Bot.ShouldExit && !_wipeCts.IsCancellationRequested)
        {
            ...
            if (!_gramielTauntLaunched)
            {
                ...
                C.Logger("[UltraGramiel-v4] All crystals dead. Launching Gramiel pulse taunt rotation.");

                string gramielFightTimeSyncPath = Ultra.ResolveSyncPath(gramielFightTimeSyncFile);
                if (_gramielTaunterIndex == 0)
                {
                    C.Logger("[UltraGramiel-v4] Gramiel Taunter (Primary) — setting fight start time.");
                    _gramielFightStart = UltraAsyncv4.SetFightTime(C, gramielFightTimeSyncPath);
                    UltraAsyncv4.StartTauntLoop(Bot, C, Engine, _gramielFightStart, 0, 4, cancellationToken: _tauntCts.Token);
                }
                else
                {
                    C.Logger($"[UltraGramiel-v4] Gramiel Taunter (Index {_gramielTaunterIndex}) — reading fight start time.");
                    _gramielFightStart = UltraAsyncv4.GetFightTime(Ultra, C, gramielFightTimeSyncPath);
                    UltraAsyncv4.StartTauntLoop(Bot, C, Engine, _gramielFightStart, _gramielTaunterIndex, 4, cancellationToken: _tauntCts.Token);
                }

                C.Logger($"[UltraGramiel-v4] Gramiel fight started — {Bot.Player.CurrentClass?.Name} taunting on pulse {_gramielTaunterIndex} of 4.");
            }
    ...
    {
        C.Logger($"[UltraGramiel-v4] CrystalTauntAsync started — 1s buffer, then 40 presses on mapId {targetMapId}.");
    ...
    {
        C.Logger($"[UltraGramiel-v4] AttackCrystalAsync started — keeping target on mapId {targetMapId} for 7s.");
        await Task.Delay(7000);
        _attackCrystal = false;
        C.Logger("[UltraGramiel-v4] AttackCrystalAsync done — returning to normal targeting.");
    }
    ...
    private void GramielMessageListener(dynamic packet)
    {
        try
        {
            ...
            foreach (dynamic anim in data.anims)
            {
                ...
                C.Logger($"[UltraGramiel-v4] Crystal warning #{tauntCounter}, turn={(currentlyT1Turn ? "T1" : "T2")}, shouldTauntNow={shouldTauntNow}");
                ...
                if (shouldTauntNow)
                {
                    ...
                    {
                        _attackCrystal = true;
                        C.Logger($"[UltraGramiel-v4] Firing CrystalTauntAsync + AttackCrystalAsync on mapId {targetCrystal}.");

...
public class Azalithv4
{
    ...
    private static UltraPotionsv4 Pots
    {
        get => _Pots ??= new UltraPotionsv4();
        set => _Pots = value;
    }
    ...
    {
        UltraGeneralv4.EquipPresetClasses(Ultra, Bot, "azalith_class-v4.sync");
    }
    ...
    void Fight()
    {
        ...
        while (!Bot.ShouldExit)
        {
            ...
            bool allDead = UltraGeneralv4.IsWholeArmyDead(Ultra, Bot, wipeDeadSyncPath);
            if (allDead)
            {
                if (!armyWipeDetected)
            ...
            if (armyWipeDetected)
            {
                bool allAlive = UltraGeneralv4.IsWholeArmyAlive(Ultra, Bot, wipeAliveSyncPath);
                if (allAlive)
                {
    ...
    void DoEnhs() => Enh.ApplyContext("azalith");
}

/*
name: LegionLichLordv4
description: Legion Lich Lord challenge boss v4 — 7 player army sync with no special mechanics.
tags: legion lich lord, challenge boss, frozenlair
*/
...
public class LegionLichLordv4
{
    ...
    public string OptionsStorage = "LegionLichLordv4";
    public List<IOption> Options = new()
    {
        ...
        new Option<int>("FarmSapphireOrbAmount", "Farm Sapphire Orb Amount", "Amount of Sapphire Orbs to farm. If 0, completes the quest once and exits.", 0),
    ...
    private void EquipPresetClasses()
    {
        ...
        SlotRequirement[] slotRequirements = UltraEnhancementsv4.GetSlotRequirements("legion lich lord", UltraClassesByRole);
        UltraCustomClassSyncv4.CustomClassSync(Ultra, Bot, slotRequirements, armySize, "legionlichlordv4_class-v4.sync", allowDuplicates);
    }
    ...
    private void Prep()
    {
        ...
        Ultra.ClearSyncFile(Ultra.ResolveSyncPath("legionlichlordv4_class-v4.sync"));
        ...
        Enh.ApplyContext("legion lich lord");
    ...
    private void Fight()
    {
        const string map = "frozenlair";
        const string boss = "Legion Lich Lord";

        const string whitemapSync = "legionlichlordv4_Whitemap.sync";
        const string mapSync = "legionlichlordv4_Map.sync";
        const string completionSyncFile = "LegionLichLordv4Completion.sync";
        int armySize = Math.Max(1, Bot.Config!.Get<int>("ArmySize"));

        const int questId = 1674;
        int targetAmount = Bot.Config!.Get<int>("FarmSapphireOrbAmount");

        if (targetAmount > 0)
            Bot.Drops.Add("Sapphire Orb");
        ...
        {
            if (targetAmount > 0 && C.CheckInventory("Sapphire Orb", targetAmount))
            {
                C.Logger($"Reached target amount of Sapphire Orb: {targetAmount}.");
                break;
            }
            ...
            while (!Bot.ShouldExit)
            {
                ...
                {
                    C.Logger("Legion Lich Lord defeated. Finishing quest.");

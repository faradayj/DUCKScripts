...
public class AstralEmpyreanv4ArmyDeathv4DevTools
{
    ...
    public string OptionsStorage = "AstralEmpyreanv4Army";
    ...
    {
        UltraGeneralv4.EquipPresetClasses(Ultra, Bot, "astralempyreanv4_class-v4.sync");
    }
    ...
    private void Fight()
    {
        ...
        while (!Bot.ShouldExit)
        {
            ...
            {
                _deathRetries++;
                C.Logger($"[AstralEmpyreanv4] Army wipe detected. Retreating. Retry {_deathRetries}/{MaxDeathRetries}.");
                ...
                UltraWaitForArmyv4.Instance.NewWaitForArmy(armySize - 1, retreatSyncFile, useSkill: false);
                C.Logger("[AstralEmpyreanv4] All retreated. Restarting fight.");
                return;
            }
            ...
            {
                _deathRetries++;
                C.Logger($"[AstralEmpyreanv4] Death detected. Waiting for army wipe. Retry {_deathRetries}/{MaxDeathRetries}.");
                ...
                while (!Bot.ShouldExit)
                {
                    ...
                    {
                        C.Logger($"[AstralEmpyreanv4] Army wipe confirmed after {(DateTime.UtcNow - wipeWaitStart).TotalSeconds:F0}s.");
                        break;
                    }

                    if ((DateTime.UtcNow - wipeWaitStart).TotalSeconds >= wipeTimeoutSec)
                    {
                        C.Logger($"[AstralEmpyreanv4] Wipe wait timeout ({wipeTimeoutSec}s) — retreating anyway.", "Warning");
                        break;
                    }

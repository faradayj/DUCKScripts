/*
name: IceWingv4
description: Warlord Icewing helper for army farming IceWing without taunter setups.
tags: Ultra, Icewing, Warlord Icewing
*/
...
public class IceWingv4
{
    ...
    private void EquipPresetClasses()
    {
        ...
        SlotRequirement[] slotRequirements = UltraEnhancementsv4.GetSlotRequirements("icewing", UltraClassesByRole);

        UltraCustomClassSyncv4.CustomClassSync(Ultra, Bot, slotRequirements, armySize, "ultra_icewing_class-v4.sync", allowDuplicates);
    }

    private void Prep()
    {
        // Clear stale class registrations from previous runs/other accounts
        Ultra.ClearSyncFile(Ultra.ResolveSyncPath("ultra_icewing_class-v4.sync"));
        ...
        Enh.ApplyContext("icewing");
    }

    private void Fight()
    {
        const string map = "icewing";
        const string boss = "Warlord Icewing";

        const string waitSyncFile = "ultra_icewing.sync";
        const string completionSyncFile = "UltraIceWingCompletion.sync";
        int armySize = 7;

        Ultra.ClearSyncFile(Ultra.ResolveSyncPath(completionSyncFile));

        C.AddDrop("Glacial Pinion");
        C.AddDrop(70052);
        Bot.Drops.Add("Glacial Pinion");
        Bot.Drops.Add(70052);
        Bot.Options.RejectAllDrops = false;
        Bot.Drops.RejectElse = false;
        ...
        Pots.EnsureRecommendedPotions(skipThird: false, context: "Icewing");

        C.Join("Whitemap");
        UltraWaitForArmyv4.Instance.NewWaitForArmy(armySize - 1, waitSyncFile, useSkill: false);

        Pots.UseRecommendedPotions(skipThird: false, ensureStock: false, context: "Icewing");
        ...
        while (!Bot.ShouldExit)
        {
            ...
            {
                C.Logger("All players acquired Glacial Pinion. Farm complete.");
                Engine.DisableSkills();
                Engine.Join(map);
    ...
    {
        return C.CheckInventory("Glacial Pinion", quant: 1, toInv: false) ||
               C.CheckInventory(70052, quant: 1, toInv: false);
    }
}

...
public class SolsticeMoonTest
{
    ...
    public CoreEnginev4 Engine = new();
    ...
    public void ScriptMain(IScriptInterface bot)
    {
        ...
        try
        {
            while (!Bot.ShouldExit)
            {
                ...
                {
                    Core.Logger($"[SolsticeMoonv4] Reached target of {TargetSliverCount} {SliverMoonlight}; stopping.");
                    break;
                }
                RunSolsticeMoon();
            }
    ...
    void ResetReusableSyncFiles()
    {
        ...
        if (!syncFilesClearedOnStartup)
        {
            Core.Logger("[Sync] Player1 is clearing startup sync files for SolsticeMoonv4.");
        ...
        Core.Logger("[Sync] Player1 is resetting reusable reset sync file for the new SolsticeMoonv4 run.");

...
public class MidnightSunTest
{
    ...
    public void ScriptMain(IScriptInterface bot)
    {
        ...
        try
        {
            while (!Bot.ShouldExit)
            {
                ...
                {
                    Core.Logger($"[MidnightSunv4] Reached target of {TargetSliverCount} {SliverSunlight}; stopping.");
                    break;
                }
                RunMidnightSun();
            }
    ...
    void ResetReusableSyncFiles()
    {
        ...
        if (!syncFilesClearedOnStartup)
        {
            Core.Logger("[Sync] Player1 is clearing startup sync files for MidnightSunv4.");
        ...
        Core.Logger("[Sync] Player1 is resetting reusable reset sync file for the new MidnightSunv4 run.");

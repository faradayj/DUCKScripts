...
public class GreatbladeOfTheEntwinedEclipse
{
    ...
    //                            └ King's Echo is used for Masakadov4.
    //                            └ Legion Revenant is used in all 3 dungeons.
    //
    //   Slot 2 (player2) needs:  StoneCrusher
    //                            └ Used for Masakadov4 AND all 3 dungeons.
    //
    //   Slot 3 (player3) needs:  ArchPaladin
    //                            └ Used for Masakadov4 (via fallback) AND all 3 dungeons.
    //
    ...
    {
        new Option<string>("player1", "King's Echo + Legion Revenant", "AQW account name for the King's Echo + Legion Revenant slot. This account MUST own BOTH classes: King's Echo is used for Masakadov4, Legion Revenant is used for the dungeons.", ""),
        new Option<string>("player2", "StoneCrusher", "AQW account name for the StoneCrusher slot. This account MUST own StoneCrusher for Masakadov4 and all dungeon phases.", ""),
        new Option<string>("player3", "ArchPaladin", "AQW account name for the ArchPaladin slot. This account MUST own ArchPaladin for the dungeon phases; Masakadov4 can also fall back to it.", ""),
        new Option<string>("player4", "Lord of Order", "AQW account name for the Lord of Order slot. This account MUST own Lord of Order for Masakadov4 and all dungeon phases.", ""),
        sArmy.packetDelay,
        ...
        new Option<bool>("tauntSanityCheck", "Check Usable Slot Before Boss Rooms", "Forwarded to AscensionoftheEclipsev4. ON: tests usable/potion slot before r3.", true),
        new Option<bool>("armyMode", "Army Mode (4 accounts)", "Forwarded to Masakadov4. ON: run Masakadov4 as a 4-account army.", true),
        new Option<bool>("allowFallbackClass", "Masakadov4: Fallback If No King's Echo", "Forwarded to Masakadov4.", true),
        new Option<bool>("useRevitalize", "Masakadov4: Use Potent Revitalize Elixir", "Forwarded to Masakadov4.", true),
        new Option<bool>("buyRevitalize", "Masakadov4: Buy Revitalize Elixir", "Forwarded to Masakadov4.", true),
    ...
    //
    //   1a. VictorMatsuriStoryv4.Storyline() runs quests 10290–10294 only. It self-skips
    ...
    //       If *any* account is missing it, ALL four run Masakadov4 together (quest
    ...
    void Phase1_VictorOfTheFestival()
    {
        ...
        new VictorMatsuriStoryv4().Storyline();

        bool anyoneNeeds = ArmyAnyoneMissing(VictorOfTheFestival, "phase1_victor_needs.sync");
        if (!anyoneNeeds)
        {
            Core.Logger($"[Phase 1] Every account already owns {VictorOfTheFestival}; skipping Masakadov4 for the whole army.");
            return;
        }

        Core.Logger($"[Phase 1b] At least one account is missing {VictorOfTheFestival}; whole army fights Masakadov4 (quest 10295 is repeatable).");
        new MasakadoKingsEchoArmy { SkipSetOptions = true }.ScriptMain(Bot);

        if (!Core.CheckInventory(VictorOfTheFestival))
        {
            Core.Logger("[Phase 1] Masakadov4 did not produce Victor of the Festival; stopping.", stopBot: true);
        }
    }
    ...
    void Phase2_FarmSlivers()
    {
        ...
        {
            Core.Logger($"[Phase 2] Already have ≥{sunlightNeeded} {SliverSunlight}; skipping MidnightSunv4 farm.");
        }
        else
        {
        ...
        {
            Core.Logger($"[Phase 2] Already have ≥{moonlightNeeded} {SliverMoonlight}; skipping SolsticeMoonv4 farm.");
        }
        else
        {

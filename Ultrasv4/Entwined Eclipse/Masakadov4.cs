...
public class MasakadoKingsEchoArmy
{
    ...
    private static CoreUltrav4 Ultra
    {
        get => _Ultra ??= new CoreUltrav4();
        set => _Ultra = value;
    }
    ...
    const string Monster = "Masakadov4";
    ...
    public void ScriptMain(IScriptInterface bot)
    {
        ...
        try
        {
            ...
            if (armyMode)
            {
                ...
                Core.Logger($"Masakadov4 army mode enabled: {sArmy.Players().Length} accounts, private room #{Core.PrivateRoomNumber}.");
            }
            else
            {
                Core.PrivateRooms = false;
                Core.Logger("Masakadov4 solo mode enabled.");
            }
    ...
    void ApplyClassEnhancements()
    {
        ...
        switch (activeClass.ToLower())
        {
            ...
            case "legion revenant":
                Core.Logger($"{Core.Username()} applying Legion Revenant Masakadov4 enhancements: Praxis, Pneuma.");
                Adv.EnhanceEquipped(
            ...
            case "archmage":
                Core.Logger($"{Core.Username()} applying ArchMage Masakadov4 enhancements: Praxis, Pneuma, Absolution.");
                Adv.EnhanceEquipped(
                    type: EnhancementType.Wizard,
                    cSpecial: CapeSpecial.Absolution,
                    hSpecial: HelmSpecial.Pneuma,
            ...
            case "arcana invoker":
                Core.Logger($"{Core.Username()} applying Arcana Invoker Masakadov4 enhancements: Praxis.");
                Adv.EnhanceEquipped(
                    type: EnhancementType.Wizard,
                    wSpecial: WeaponSpecial.Praxis
                );
    ...
    void OpenWithPraxis()
    {
        ...
        Core.Logger($"{Core.Username()} opening Masakadov4 with Praxis on AQW button 2 / Skua skill 1 using {activeClass}.");
    ...
    void OpenWithDefense()
    {
        ...
        // Masakadov4 uses Unsheathe once at the start, then Yokai Slayer 3s later.

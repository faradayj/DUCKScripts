...
public class UnifiedQueuev4
{
    public static IEnumerable<string> GetSharedBossQueue(dynamic ultra, IScriptInterface bot, dynamic C, IEnumerable<string> bosses, string bossSyncFile, string participantSyncFile, Func<string, bool> isBossComplete, int armySize = 4)
    {
        if (bot == null)
            return bosses;
        ...
        while (!bot.ShouldExit)
        {
            ...
            {
                lastCount = count;
                C.Logger($"[UnifiedQueue-v4] Registered participants: {count}/{armySize}", "Info");
            }
    ...
    private static void CleanupStaleParticipants(dynamic ultra, dynamic C, string path)
    {
        try
        {
            ...
            if (freshLines.Count != lines.Length)
            {
                ...
                C.Logger($"[UnifiedQueue-v4] Cleaned stale participant entries; {freshLines.Count} remain.", "Info");
            }
        }
        catch (Exception ex)
        {
            C.Logger($"[UnifiedQueue-v4] Failed to cleanup participant sync: {ex.Message}", "Warning");
        }
    }
}

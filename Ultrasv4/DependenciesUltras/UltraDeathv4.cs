...
public class UltraDeathv4
{
    private IScriptInterface Bot => IScriptInterface.Instance;
    private CoreBots C => CoreBots.Instance;
    private CoreUltrav4 Ultra => _Ultra ??= new CoreUltrav4();
    private CoreUltrav4 _Ultra;
    ...
    public async Task MonitorDeathAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    SignalDeath();
                    C.Logger("[UltraDeathv4] Death detected, signaling army.");
                }
    ...
    )
    {
        ...
        while (!wipeCts.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    C.Logger("[UltraDeathv4] Army wipe detected — running retreat.");
                    wipeCts.Cancel();
            ...
            {
                C.Logger($"[UltraDeathv4] WipeMonitor error: {ex.Message}");
            }
            finally
            {
    ...
    )
    {
        ...
        C.Logger($"[UltraDeathv4] Retreating. Retry {currentRetries}/{maxRetries}.");

new CoreUltrav4().PersistentJoinHouse();

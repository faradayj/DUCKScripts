...
public class CoreUltrav4
{
    public IScriptInterface Bot => IScriptInterface.Instance;
    public CoreEnginev4 Core => CoreEnginev4.Instance;
   ...
   )
    {
        ...
        if (armySize > 0 && activeMembers < armySize)
            return false;

        return activeMembers > 0 && completedMembers == activeMembers;
    }

    public bool CheckArmyProgressBool(Func<bool> condition, string syncFilePath = "army_sync.sync", int armySize = 0)
    {
        if (!Bot.Player.Alive)
        {
            Bot.Wait.ForTrue(() => Bot.Player.Alive, 20);
    ...
    public void JoinHouse()
    {
        ...
        try
        {
            if (Bot.House.Items.Any(h => h.Equipped))
            {
                ...
                if (!string.Equals(Bot.Map.Name, "house", StringComparison.OrdinalIgnoreCase))
                {
                    C.Logger("[CoreUltrav4] House join did not succeed after retries. Staying in current map.");
                }
            }
            else
            {
                C.Logger("[CoreUltrav4] No house available, falling back to Yulgar.");
                C.Join("yulgar-100000");
            }
        }
        catch (Exception ex)
        {
            C.Logger($"[CoreUltrav4] JoinHouse failed: {ex.Message}. Falling back to Yulgar.");
            C.Join("yulgar-100000");
        }
    }
    ...
    public void PersistentJoinHouse()
    {
        ...
        while (!Bot.ShouldExit)
        {
            ...
            {
                C.Logger($"[CoreUltrav4] PersistentJoinHouse failed: {ex.Message}");
            }
